...
App.utils = (function () {
  ...
  // Same idea as safeTry, but for async functions. A plain try/catch around
  // an async function only catches errors thrown before the first await —
  // anything that rejects after that becomes an unhandled promise rejection
  // instead of hitting the catch block. This awaits the result so rejections
  // are caught properly too.
  async function safeTryAsync(fn, fallback) {
    try {
      return await fn();
    } catch (e) {
      App.utils.logError("safeTryAsync", e);
      return typeof fallback === "function" ? fallback(e) : fallback;
    }
  }
  ...
  return { escapeHtml, qs, qsa, debounce, safeTry, safeTryAsync, logError, detectDeviceType };
})();
